...
from .utils import compute_average_precision_detection, compute_temporal_iou_batch_cross, compute_temporal_iou_batch_paired, get_ap, load_jsonl
...
def compute_mr_ap(
    submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10), max_gt_windows=None, max_pred_windows=10, num_workers=8, chunksize=50
):
    ...
    for d in submission:
        ...
        for w in pred_windows:
            pred_qid2data[qid].append({"video-id": d["qid"], "t-start": w[0], "t-end": w[1], "score": w[2]})  # in order to use the API
    ...
    if num_workers > 1:
        ...
        # using joblib
        # qid2ap_list = dict(Parallel(n_jobs=num_workers)(delayed(compute_ap_from_triple)(data_triple) for data_triple in data_triples))
    ...
    for d in ground_truth:
        ...
        if len(cur_gt_windows) > 0:
            try:  # todo , need to fix for ego4d.
                ...
                print(f"error happen when `cur_ious' equal to {cur_ious}, Exception: {e}")
...
def compute_hl_ap(qid2preds, qid2gt_scores_binary, num_workers=8, chunksize=50):
    ...
    if num_workers > 1:
        with mp.Pool(num_workers) as pool:
            for idx, w_idx, score in pool.imap_unordered(compute_ap_from_tuple, input_tuples, chunksize=chunksize):
                ap_scores[idx, w_idx] = score
        # using joblib to avoid out of memory issue, which caused by mp.Pool using fork.
        # ap_scores = np.array(Parallel(n_jobs=num_workers)(delayed(compute_ap_from_tuple)(input_tuple) for input_tuple in input_tuples))
    else:
...
def eval_highlight(submission, ground_truth, verbose=True):
    ...
    for gt_saliency_score_min, score_name in zip(gt_saliency_score_min_list, saliency_score_names):
        start_time = time.time()
        qid2gt_scores_binary = {k: (v >= gt_saliency_score_min).astype(float) for k, v in qid2gt_scores_full_range.items()}  # scores in [0, 1]
...
def vtg_eval(submission, ground_truth, verbose=True, match_number=True):
    ...
    if match_number:
        assert pred_qids == gt_qids, "qids in ground_truth and submission must match. " "use `match_number=False` if you wish to disable this check"
    ...
    if ("pred_saliency_scores" in submission[0]) and ("saliency_scores" in ground_truth[0]):
        if isinstance(ground_truth[0]["saliency_scores"], list):  # some item might be nan not list.
            ...
            highlight_det_scores_brief = dict([(f"{k}-{sub_k.split('-')[1]}", v[sub_k]) for k, v in highlight_det_scores.items() for sub_k in v])
