...
def load_jsonl(filename):
    with open(filename, "r") as f:
        return [json.loads(file_t.strip("\n")) for file_t in f.readlines()]
...
def compute_temporal_iou_batch_paired(pred_windows, gt_windows):
    ...
    """
    intersection = np.maximum(0, np.minimum(pred_windows[:, 1], gt_windows[:, 1]) - np.maximum(pred_windows[:, 0], gt_windows[:, 0]))
