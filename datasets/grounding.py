    ...
    def evaluate(
        ...
        for blob in blobs:
            ...
            if "boundary" in blob:
                ...
                # here need to remove the un-normal values in bnd. (null, inf, nan)
                bnd = bnd[~torch.isnan(bnd).any(dim=1)]
                bnd = bnd[~torch.isinf(bnd).any(dim=1)]
                bnd = bnd[bnd[:, 0] < bnd[:, 1]]
                # remove the windows begin less than 0
                bnd = bnd[bnd[:, 0] >= 0]
            ...
            if "saliency" in blob:
                # here need to remove the un-normal values in saliency. (null, inf, nan)
                blob["saliency"] = blob["saliency"][~torch.isnan(blob["saliency"])]
        ...
        # save out to json file, this operation just for the visualization in paper.
        # out_path = self.label_path.replace(".json", "_submission_{}.json".format(nncore.get_timestamp()))
        # nncore.dump(out, out_path)
