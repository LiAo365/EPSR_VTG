        ...
        for s in strides:
            ...
            if p == 0:
                ...
                for _ in range(abs(p)):
                    layers.extend([Permute(), conv_cls(dims, dims, 2, stride=2), Permute(), nn.LayerNorm(dims), nn.ReLU(inplace=True)])
...
class AdaPooling(nn.Module):
    ...
    def forward(self, x, mask):
        a = self.att(x) + torch.where(mask.unsqueeze(2) == 1, 0.0, float("-inf"))
        a = a.softmax(dim=1)
...
class ConvHead(nn.Module):
    ...
    def __init__(self, dims, out_dims, kernel_size=3):
        super(ConvHead, self).__init__()

        # yapf:disable
        self.module = nn.Sequential(
            Permute(),
            nn.Conv1d(dims, dims, kernel_size, padding=kernel_size // 2),
            nn.ReLU(inplace=True),
            nn.Conv1d(dims, out_dims, kernel_size, padding=kernel_size // 2),
            Permute(),
        )
        # yapf:enable
